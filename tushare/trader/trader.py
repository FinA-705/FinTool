...
class AsyncTraderAPI(object):
    ...
    async def login(self):
        ...
        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            # 获取登录页面
            async with session.get(vs.CSC_PREFIX % (vs.P_TYPE['https'], vs.DOMAINS['csc'],
                                                    vs.PAGES['csclogin']), headers=headers) as resp:
                pass
            
            # 获取验证码
            async with session.get(vs.V_CODE_URL % (vs.P_TYPE['https'],
                                                    vs.DOMAINS['cscsh'],
                                                    vs.PAGES['vimg'])) as resp:
                v_code_content = await resp.read()
                
            # 这里需要处理验证码，简化处理
            v_code = utils.get_vcode('csc', type('MockResponse', (), {'content': v_code_content})())
            
            if await self._login(session, v_code):
                return True
            else:
                print('请确认账号或密码是否正确，或券商服务器是否处于维护中。')
                return False
                
    async def _login(self, session, v_code):
        """异步登录处理"""
        brokerinfo = up.get_broker(self.broker)
        user = brokerinfo['user'][0]
        login_params = dict(
        ...
        async with session.post(vs.CSC_LOGIN_ACTION % (vs.P_TYPE['https'], 
                                                       vs.DOMAINS['csc']), 
                                params=login_params) as logined:
            text = await logined.text()
            return text.find(u'消息中心') != -1
    
    async def buy(self, stkcode, price=0, count=0, amount=0):
        """异步买入证券"""
        result = await self._trading(stkcode, price, count, amount, 'B', 'buy')
        if result:
            jsonobj = utils.get_jdata(result)
            return True if jsonobj['result'] == 'true' else False
        return False
        
    async def sell(self, stkcode, price=0, count=0, amount=0):
        """异步卖出证券"""
        result = await self._trading(stkcode, price, count, amount, 'S', 'sell')
        if result:
            jsonobj = utils.get_jdata(result)
            return True if jsonobj['result'] == 'true' else False
        return False
    
    async def _trading(self, stkcode, price, count, amount, tradeflag, tradetype):
        """异步交易处理"""
        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            # 获取交易检查信息
            url = vs.TRADE_CHECK_URL % (vs.P_TYPE['https'], 
                                        vs.DOMAINS['csc'], 
                                        vs.PAGES['tradecheck'],
                                        tradeflag, stkcode, 
                                        tradetype, utils.nowtime_str())
            async with session.get(url) as resp:
                txtdata = await resp.text()
                
            jsonobj = utils.get_jdata(type('MockResponse', (), {'text': txtdata})())
            list = jsonobj['returnList'][0]
            secuid = list['buysSecuid']
            stkname = list['stkname']
            ...
            if secuid is not None:
                ...
                async with session.post(vs.TRADE_URL % (vs.P_TYPE['https'], 
                                                        vs.DOMAINS['csc'], 
                                                        vs.PAGES['trade']), 
                                        params=tradeparams) as trade_resp:
                    return await trade_resp.text()
            return None
